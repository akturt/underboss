...
4. **For new documents check that author started from template:**
   - Compare structure to corresponding `templates/<type>.md` in Runtime.
...
- Don't run on files outside `docs/**/*.md` (templates/, schemas/, etc. live in Runtime submodule and are out of scope for consumer PR review).
