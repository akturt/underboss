---
schema: 1
id: agent-claude-code-documentation-reviewer
type: prompt
status: active
date: 2026-07-08
...
This agent runs on every PR that contains changes to `docs/**/*.md`. Invoked by CI, by `@claude-code documentation-reviewer review`, or by request when author requests pre-submit check.
...
3. **For each changed file, verify Schema v1 conformance:**
   ...
   - `status` per type (see `playbook/playbook-v2.md` §Status enum):
...
4. **For new documents check that author started from template:**
   - Compare structure to corresponding `engine/templates/<type>.md` in Runtime.
...
### Read context
- playbook/playbook-v2.md: sections reviewed
...
- **low**: informational, does not block — APPROVED with note. Examples: `updated:` not bumped but content changed, `tags` could be more descriptive, markdown formatting isize drift.
...
- Don't run on files outside `docs/**/*.md` (engine/templates/, engine/schemas/, etc. live in Runtime submodule and are out of scope for consumer PR review).
