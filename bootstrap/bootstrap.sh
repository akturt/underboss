...
if [ ! -f "$ENTITY_CATALOG" ] && [ -f "$ENTITY_CATALOG_TEMPLATE" ]; then
  ...
  echo "Created docs/architecture/entity-catalog.md (sections pre-filled)"
fi

# ─── 1c. v1.3: Auto-create architecture/README.md from project structure ─────

ARCH_README="$TARGET/docs/architecture/README.md"
if [ ! -f "$ARCH_README" ]; then
  # Scan top-level directories for topology
  DIRS_LIST=""
  if [ -d "$TARGET" ]; then
    DIRS_LIST=$(find "$TARGET" -maxdepth 1 -type d -not -path "*/\.*" -not -path "$TARGET" | while read -r d; do
      basename "$d"
    done | sort | head -20)
  fi
  ...
  CONFIGS=""
  for f in docker-compose.yml docker-compose.yaml Dockerfile Makefile pyproject.toml package.json go.mod Cargo.toml; do
    if [ -f "$TARGET/$f" ]; then
      CONFIGS="$CONFIGS- $f\n"
    fi
  done

  cat > "$ARCH_README" << README
# Architecture

> Auto-generated by bootstrap v1.3. Update this file with your project topology and invariants.

## Topology

$(echo "$DIRS_LIST" | while read -r d; do [ -n "$d" ] && echo "- \`$d/\`"; done)

## Configuration Files

$(echo -e "$CONFIGS" | sed '/^$/d')

## Invariants

<!-- Add critical invariants below -->
<!-- Format: - INV-1: description -->

## Boundaries

<!-- Describe system boundaries -->
- **Runtime**: \`docs/.runtime/naprolom-docs/\` (pristine, managed by git submodule)

---

**Status:** auto-generated — update with actual architecture documentation.
README
  echo "Created docs/architecture/README.md (auto-generated from project structure)"
fi

# ─── 1d. v1.3: Auto-detect stack from lockfiles ────────────────────────────
...
detect_stack() {
  ...
  if [ -f "$TARGET/package.json" ]; then
    local name
    name=$(grep -o '"name"[[:space:]]*:[[:space:]]*"[^"]*"' "$TARGET/package.json" 2>/dev/null | head -1 | sed 's/.*: *"//; s/".*//')
  ...
  if [ -f "$TARGET/pyproject.toml" ]; then
    if grep -q "fastapi" "$TARGET/pyproject.toml" 2>/dev/null; then
      backend="FastAPI"
    elif grep -q "django" "$TARGET/pyproject.toml" 2>/dev/null; then
      backend="Django"
    elif grep -q "flask" "$TARGET/pyproject.toml" 2>/dev/null; then
      backend="Flask"
    else
      backend="Python"
    fi
    ...
    if grep -qi "fastapi" "$TARGET/requirements.txt" 2>/dev/null; then
      backend="FastAPI"
    elif grep -qi "django" "$TARGET/requirements.txt" 2>/dev/null; then
      backend="Django"
    elif grep -qi "flask" "$TARGET/requirements.txt" 2>/dev/null; then
      backend="Flask"
    else
      backend="Python"
    fi
  ...
  if [ -f "$TARGET/docker-compose.yml" ] || [ -f "$TARGET/docker-compose.yaml" ]; then
    ...
    if grep -q "postgres" "$dc" 2>/dev/null; then
      database="PostgreSQL"
    elif grep -q "mysql" "$dc" 2>/dev/null; then
      database="MySQL"
    elif grep -q "mongo" "$dc" 2>/dev/null; then
      database="MongoDB"
    elif grep -q "redis" "$dc" 2>/dev/null; then
      database="Redis"
    fi
  fi
  ...
  if [ -f "$TARGET/vite.config.ts" ] || [ -f "$TARGET/vite.config.js" ] || [ -f "$TARGET/vite.config.mjs" ]; then
    infrastructure="$infrastructure Vite"
  fi
...
IFS='|' read -r STACK_BACKEND STACK_DATABASE STACK_INFRA <<< "$STACK_INFO"

# ─── 1e. v1.3: Auto-generate boundaries.yml from .gitignore ────────────────

generate_boundaries() {
  local pristine_paths="" editable_paths="" generated_paths=""

  # Always pristine
  pristine_paths="    - path: docs/.runtime/\n      reason: Documentation System Runtime submodule (managed by git submodule update --remote)"

  # Scan .gitignore for generated patterns
  if [ -f "$TARGET/.gitignore" ]; then
    while IFS= read -r line; do
      ...
      # Match known generated patterns
      case "$pattern" in
        node_modules|vendor|dist|build|coverage|.next|.nuxt|__pycache__|.venv|venv)
          generated_paths="$generated_paths    - path: $pattern/\n      reason: generated/dependency (auto-detected from .gitignore)\n"
          ;;
      esac
    done < "$TARGET/.gitignore"
  fi

  # Common generated patterns if not in .gitignore
  for p in node_modules dist build coverage __pycache__ .venv vendor; do
    if [ -d "$TARGET/$p" ] && ! echo "$generated_paths" | grep -q "path: $p/"; then
      generated_paths="$generated_paths    - path: $p/\n      reason: generated (auto-detected from directory presence)\n"
    fi
  done

  echo -e "$pristine_paths"
...
if [ ! -f "$TARGET/.context/project.yml" ]; then
  # Auto-detect project name from package.json / pyproject.toml / go.mod
  if [ -z "$PROJECT_NAME" ]; then
    if [ -f "$TARGET/package.json" ]; then
      PROJECT_NAME=$(grep -o '"name"[[:space:]]*:[[:space:]]*"[^"]*"' "$TARGET/package.json" 2>/dev/null | head -1 | sed 's/.*: *"//; s/".*//')
    elif [ -f "$TARGET/pyproject.toml" ]; then
      PROJECT_NAME=$(grep -E "^name" "$TARGET/pyproject.toml" 2>/dev/null | head -1 | sed 's/.*=[[:space:]]*//; s/"//g')
    elif [ -f "$TARGET/go.mod" ]; then
      PROJECT_NAME=$(grep "^module" "$TARGET/go.mod" 2>/dev/null | head -1 | sed 's/module[[:space:]]*//')
    fi
  fi
  [ -z "$PROJECT_NAME" ] && PROJECT_NAME="TODO-project-name"

  # Build stack arrays
  STACK_BACKEND="["
  [ -n "$STACK_BACKEND" ] && STACK_BACKEND="$STACK_BACKEND$STACK_BACKEND"
...
directories:
  key: {}
YML
  echo "Created .context/project.yml (stack auto-detected)"
fi

if [ ! -f "$TARGET/.context/boundaries.yml" ]; then
  # Generate boundaries from .gitignore patterns
  BOUNDARIES_CONTENT="boundaries:\n  pristine:\n    - path: docs/.runtime/\n      reason: Documentation System Runtime submodule (managed by git submodule update --remote)\n  editable:\n    - path: docs/\n      reason: all user-authored documentation (architecture, adr, specs, audits, backlog, api)\n  generated:\n"

  # Add generated paths from .gitignore
  for p in node_modules dist build coverage __pycache__ .venv vendor .next .nuxt; do
    if [ -d "$TARGET/$p" ] || ( [ -f "$TARGET/.gitignore" ] && grep -q "^$p$" "$TARGET/.gitignore" 2>/dev/null ) || ( [ -f "$TARGET/.gitignore" ] && grep -q "^/$p$" "$TARGET/.gitignore" 2>/dev/null ); then
      BOUNDARIES_CONTENT="$BOTTOMARIES_CONTENT    - path: $p/\n      reason: generated (auto-detected from .gitignore)\n"
    fi
  done

  BOUNDARIES_CONTENT="$BOUNDARIES_CONTENT  secret: []"

  echo -e "$BOUNDARIES_CONTENT" > "$TARGET/.context/boundaries.yml"
  echo "Created .context/boundaries.yml (patterns auto-detected from .gitignore)"
fi
...
echo ""
echo "Auto-generated:"
echo "  - docs/architecture/README.md (from project structure)"
echo "  - .context/project.yml (stack auto-detected)"
echo "  - .context/boundaries.yml (patterns from .gitignore)"
echo "  - docs/architecture/entity-catalog.md (sections pre-filled)"
echo ""
echo "Next steps:"
